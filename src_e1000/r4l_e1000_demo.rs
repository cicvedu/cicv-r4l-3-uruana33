...
impl net::DeviceOperations for NetDevice {
    ...
    fn open(dev: &net::Device, data: &NetDevicePrvData) -> Result {
        pr_info!("进入 NetDevice is a net::DeviceOperations 逻辑, 启用网卡\n");
        ...
        *data.tx_ring.lock_irqdisable() = Some(tx_ringbuf);
        ...
        pr_info!("进入 NetDevice is a net::DeviceOperations 逻辑, 启用完成\n");

        Ok(())
    }

    fn stop(_dev: &net::Device, _data: &NetDevicePrvData) -> Result {
        pr_info!("进入 NetDevice is a net::DeviceOperations 逻辑, 禁用网卡\n");
        pr_info!("Rust for linux e1000 driver demo (net device stop)\n");

        pr_info!("----------e1000_reset_hw------------\n");
        _data.e1000_hw_ops.e1000_reset_hw()?;
        let status = _data.e1000_hw_ops.e1000_read_interrupt_state();
        pr_info!("----------status {}------------\n", status);
        ...
        // 释放irq中断处理资源，
        // 获取旧指针并将原子指针设为 null
        let old_ptr = _data._irq_handler.swap(core::ptr::null_mut(), core::sync::atomic::Ordering::SeqCst); 
        if !old_ptr.is_null() {
...
impl driver::DeviceRemoval for E1000DrvPrvData {
    fn device_remove(&self) {
        pr_info!("进入E1000DrvPrvData device_remove 逻辑\n");
        pr_info!("Rust for linux e1000 driver demo (device_remove)\n");
        // let pci_dev: &mut pci::Device =
        //     unsafe { &mut *(net_dev.raw_device() as *mut pci::Device) };

        // // // let pci_dev = _data.dev.raw_device() as *mut pci::Device;
        // // let bars = pci_dev.select_bars((bindings::IORESOURCE_MEM | bindings::IORESOURCE_IO) as u64);
        // // pr_info!("----------select bars {}------------\n", bars);

        // // 禁用网卡
        // pr_info!("----------disable device------------\n");
        // pci_dev.disable_device();
        // pr_info!("----------disable device finished------------\n");

        // // 释放资源
        // pr_info!("----------release_selected_regions------------\n");
        // pci_dev.release_selected_regions(self._bars);

        // 其余逻辑在stop中
        // 会自动drop net_dev
        // net_dev.priv_flags_set(E1000_CTRL_RST);
    }
}
...
impl pci::Driver for E1000Drv {
    ...
    fn probe(dev: &mut pci::Device, id: core::option::Option<&Self::IdInfo>) -> Result<Self::Data> {
        ...
        let status = e1000_hw_ops.e1000_read_interrupt_state();
        pr_info!("----------on probe, status {}------------\n", status);
